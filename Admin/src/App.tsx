...
export default function App() {
  ...
  useEffect(() => {
    localStorage.removeItem('urbanpark_users');
    const existingUsers = localStorage.getItem('urbanpark_users_v2');
    let usersList = existingUsers ? JSON.parse(existingUsers) : [];

    const hasAdmin = usersList.some((u: any) => u.phone === '0901234567');
    const hasUser = usersList.some((u: any) => u.phone === '0908888888');

    if (!existingUsers || !hasAdmin || !hasUser) {
      usersList = [
        {
          phone: '0901234567',
          password: '123456',
          name: 'Nguyễn Văn Đạt (ADMIN)',
        ...
        },
        {
          phone: '0908888888',
          password: '123456',
          name: 'Trần Thị Nhân Viên (USER)',
          email: 'user.demo@urbanpark.com',
          role: 'USER'
        }
      ];
      localStorage.setItem('urbanpark_users_v2', JSON.stringify(usersList));
    }
    ...
    if (storedRefreshToken && storedUser) {
      try {
        ...
        // Force migration of session user if role is legacy
        if (parsedUser.role !== 'ADMIN' && parsedUser.role !== 'USER') {
          parsedUser.role = 'USER';
          localStorage.setItem('urbanpark_session_user', JSON.stringify(parsedUser));
        }
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('urbanpark_users_v2', JSON.stringify(userList));
