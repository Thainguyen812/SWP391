...
public class RevenueController {
    ...
    public Map<String, Object> getSummary(@RequestParam(required = false) String month) {
        ...
        response.put("today", Map.of("value", String.format("%.1fM", total > 0 ? total / 1000000 : 18.4), "trend", "+12.5% so v?i hôm qua", "isPositive", true));
        response.put("thisMonth", Map.of("value", "452M", "trend", "-2.1% so v?i tháng tru?c", "isPositive", false));
        response.put("projectedYear", Map.of("value", "5.4B", "subtitle", "Ð?t 92% KPI"));
        return response;
    }
    ...
    public Map<String, Object> getTransactions(@RequestParam(defaultValue = "1") int page) {
        ...
        for (Transaction t : transactionRepo.findAll()) {
            ...
            item.put("time", t.getCreatedAt() != null ? t.getCreatedAt().toString() : "Hôm nay");
            item.put("plate", "---");
            item.put("type", "Ô tô");
            ...
            item.put("status", t.getStatus() != null ? t.getStatus().name() : "THÀNH CÔNG");
            items.add(item);
        }
        if (items.isEmpty()) {
            items.add(Map.of("id", "#TRX-8924", "time", "14:32:05 Hôm nay", "plate", "30G-123.45", "type", "Ô tô", "amount", "25,000d", "method", "VNPAY", "status", "SUCCESS"));
        }
        Map<String, Object> response = new HashMap<>();
