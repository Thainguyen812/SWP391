...
public interface ParkingSessionRepository extends JpaRepository<ParkingSession, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("UPDATE ParkingSession p SET p.isLocked = :locked WHERE p.vehicleId = :vehicleId AND p.sessionStatus = :status")
    int updateLockStatusByVehicleId(
            @org.springframework.data.repository.query.Param("vehicleId") UUID vehicleId,
            @org.springframework.data.repository.query.Param("status") ParkingSession.SessionStatus status,
