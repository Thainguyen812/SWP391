...
public class ParkingServiceImpl implements ParkingService {
        ...
        public CheckInResponse aiCheckIn(AiCheckInRequest request) {
                ...
                if (existing.isPresent()) {
                        throw new ApiExceptions.ConflictException(
                                        "Active parking session already exists for this vehicle");
                }
        ...
        public void verifyExitQr(String detectedPlate, String qrToken) {
                ...
                Vehicle vehicle = vehicleRepository.findById(qr.getVehicleId())
                                .orElseThrow(() -> new ApiExceptions.BadRequestException(
                                                "Phương tiện liên kết với QR không tồn tại!"));
                ...
                ParkingSession session = parkingSessionRepository.findByLicensePlateAndSessionStatus(
                                ...
                                ParkingSession.SessionStatus.ACTIVE).orElseThrow(
                                                () -> new ApiExceptions.NotFoundException(
                                                                "Không tìm thấy phiên gửi xe hoạt động cho xe này!"));
                ...
                if (session.getIsLocked() != null && session.getIsLocked()) {
                        ...
                        System.out.println("MOCK FCM PUSH NOTIFICATION: [CẢNH BÁO CHỐNG TRỘM] Phát hiện xe "
                                        + qrVehiclePlate
                ...
                if (!scanLogs.isEmpty()) {
                        ...
                        if (vehicle.getColorRgb() != null && latestLog.getDetectedColorRgb() != null) {
                                colorDiff = calculateColorDifference(vehicle.getColorRgb(),
                                                latestLog.getDetectedColorRgb());
                                if (colorDiff > 30.0) {
                                        colorMismatch = true;
                                }
                        }
                        ...
                        if (shapeMismatch || colorMismatch) {
                                ...
                                session.setSuspiciousReason("FINGERPRINT_MISMATCH: "
                                                + (shapeMismatch ? "Sai kiểu dáng xe. " : "")
                                ...
                                audit.setEntityId(session.getId());
                                audit.setOldValue(
                                                "{\"color\":\"" + vehicle.getColorRgb() + "\", \"shape\":\""
                                                                + vehicle.getBodyShape() + "\"}");
                ...
                VipSubscription sub = vipSubscriptionRepository
                                ...
                                .orElseThrow(() -> new ApiExceptions.BadRequestException(
                                                "Xe không có gói cước VIP hoạt động!"));
                ...
                Zone zone = zoneRepository.findById(session.getAssignedZoneId())
                                .orElseThrow(
                                                () -> new ApiExceptions.BadRequestException(
                                                                "Không tìm thấy tầng đỗ liên kết với phiên gửi!"));
        ...
        public CheckInResponse visitorCheckIn(VisitorCheckInRequest request) {
                ...
                // 2. Tìm thẻ
                Card card = cardRepository.findByCardCode(request.getCard_code())
                ...
                if (blacklistRepository.existsByCardId(card.getId())) {
                        throw new ApiExceptions.ForbiddenException("Thẻ này đang nằm trong blacklist");
                }

                // 3. Ánh xạ loại xe từ request sang slot_type của DB
                String slotType = mapVehicleToSlotType(request.getVehicle_type());

                // 4. Tìm zone phù hợp và Slot cụ thể trong zone đó
                List<Zone> candidates = zoneRepository.findAll();
                Zone chosen = null;
                ParkingSlot targetSlot = null;

                for (Zone z : candidates) {
                        if (z.getAllowedSizes() != null && z.getAllowedSizes().contains(request.getVehicle_type())) {
                                // Tìm slot trống trong zone thỏa mãn điều kiện slotType
                                Optional<ParkingSlot> availableSlot = parkingSlotRepository
                                                .findFirstByZoneIdAndSlotStatusAndSlotType(z.getId(), "AVAILABLE",
                                                                slotType);

                                if (availableSlot.isPresent()) {
                                        chosen = z;
                                        targetSlot = availableSlot.get();
                                        break;
                                }
                        }
                }

                if (chosen == null || targetSlot == null) {
                        throw new ApiExceptions.BadRequestException("Không còn slot phù hợp (loại " + slotType
                                        + ") trong các zone cho loại xe này");
                }

                // 5. Cập nhật trạng thái slot và zone
                targetSlot.setSlotStatus("OCCUPIED");
                targetSlot.setLastUpdated(Instant.now());
                parkingSlotRepository.save(targetSlot);

                zoneRepository.increaseOccupied(chosen.getId());

                // 6. Tạo hoặc tìm Vehicle
                Vehicle vehicle = vehicleRepository.findByLicensePlate(plate)
                                .orElseGet(() -> {
                                        Vehicle newVehicle = new Vehicle();
                                        newVehicle.setId(UUID.randomUUID());
                                        newVehicle.setOwnerId(UUID.fromString("a0000000-0000-0000-0000-000000000005"));
                                        newVehicle.setLicensePlate(plate);
                                        newVehicle.setVehicleSize(request.getVehicle_type());
        ...
        // Hàm ánh xạ logic (Đặt ở cuối class ParkingServiceImpl)
        private String mapVehicleToSlotType(String vehicleType) {
                if (vehicleType == null)
                        return "NORMAL";
                return switch (vehicleType.toUpperCase()) {
                        case "SEDAN", "SUV", "HATCHBACK", "FAMILY_CAR", "VAN_TRUCK", "MINIBUS_16" -> "NORMAL";
        ...
        public Transaction checkoutCard(UUID cardId) {
                ...
                if (session.getSessionStatus() == ParkingSession.SessionStatus.PASSED_CONFIRMED) {
                        ...
                        Instant expireTime = session.getMobileCheckoutAt()
                                        .plusSeconds(1800);
                ...
                if (blacklistRepository.existsByCardId(cardId)) {
                        throw new ApiExceptions.ForbiddenException(
                                        "Thẻ này đang nằm trong blacklist, không thể checkout");
                }
                ...
                Vehicle vehicle = vehicleRepository.findByLicensePlate(session.getLicensePlate())
                                .orElseThrow(() -> new ApiExceptions.NotFoundException(
                                                "Không tìm thấy thông tin xe để tính phí"));
