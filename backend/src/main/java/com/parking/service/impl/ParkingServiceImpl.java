...
public class ParkingServiceImpl implements ParkingService {
    ...
    @Override
    @Transactional
    public Transaction congestionCheckout(com.parking.dto.CongestionCheckoutRequest request) {
        ParkingSession session = parkingSessionRepository
                .findByLicensePlateAndSessionStatus(
                        request.getLicensePlate(),
                        ParkingSession.SessionStatus.ACTIVE)
                .orElseThrow(() -> new ApiExceptions.NotFoundException("Không tìm thấy phiên gửi xe ACTIVE"));

        if (session.getIsVip() != null && session.getIsVip()) {
            throw new ApiExceptions.BadRequestException("Xe VIP không thuộc luồng congestion checkout");
        }
        ...
        transaction.setIsMobileCheckout(true);
        transaction.setMobileGpsLocation(request.getGpsLocation());
        transaction.setMobilePhotoProof(request.getProofImageUrl());
        transaction.setProcessedBy(request.getStaffId());
        transaction.setProcessedAt(checkoutTime);
        transaction = transactionRepository.save(transaction);

        session.setMobileCheckoutStaffId(request.getStaffId());
        session.setMobileCheckoutLocation(request.getGpsLocation());
        session.setMobileCheckoutPhoto(request.getProofImageUrl());
        session.setMobileCheckoutAt(checkoutTime);
        session.setSessionStatus(ParkingSession.SessionStatus.PASSED_CONFIRMED);
        parkingSessionRepository.save(session);
