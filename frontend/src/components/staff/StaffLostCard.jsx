...
export const StaffLostCard = () => {
  ...
  const [searchPlate, setSearchPlate] = useState(currentVehicle?.plate || "");
  const [foundVehicle, setFoundVehicle] = useState(currentVehicle || null);
  ...
  const handleCaptureCccd = () => {
    ...
    setTimeout(() => {
      setCccdImage("https://images.unsplash.com/photo-1633265486064-086b219458ce?auto=format&fit=crop&w=600&q=80");
  ...
  const handleCaptureReg = () => {
    notification.info({ message: 'Đang mở camera...', duration: 1 });
    setTimeout(() => {
      setRegImage("https://images.unsplash.com/photo-1588681664899-f142ff2dc9b1?auto=format&fit=crop&w=600&q=80");
            ...
            {foundVehicle ? (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mt-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="text-slate-500 text-xs font-medium">Mã phiên: #TRX-{Math.floor(10000 + Math.random() * 90000)}</div>
                </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="relative bg-slate-200 rounded-lg aspect-video flex items-center justify-center overflow-hidden border border-slate-300">
                {foundVehicle?.image ? (
                  <img src={foundVehicle.image} alt="Front Left" className="w-full h-full object-cover opacity-90" />
                ) : (
              ...
              <div className="relative bg-slate-200 rounded-lg aspect-video flex items-center justify-center overflow-hidden border border-slate-300">
                {foundVehicle?.image ? (
                  <img src={foundVehicle.image} alt="Front Right" className="w-full h-full object-cover opacity-90" />
                ) : (
                  <div className="text-slate-400 text-xs">Chưa có dữ liệu</div>
                )}
                <div className="absolute top-2 left-2 bg-slate-800/80 backdrop-blur-sm text-white text-[9px] font-bold px-2 py-1 rounded uppercase tracking-wider">
              ...
              <div className="relative bg-slate-200 rounded-lg aspect-video flex items-center justify-center overflow-hidden border-2 border-red-300">
                <img src={foundVehicle?.image || "https://images.unsplash.com/photo-1580273916550-e323be2ae537?auto=format&fit=crop&w=600&q=80"} alt="Rear Left Live" className="w-full h-full object-cover opacity-90" />
              ...
              {/* Image 4 (LIVE) */}
              <div className="relative bg-slate-200 rounded-lg aspect-video flex items-center justify-center overflow-hidden border-2 border-red-300">
                <img src={foundVehicle?.image || "https://images.unsplash.com/photo-1580273916550-e323be2ae537?auto=format&fit=crop&w=600&q=80"} alt="Rear Right Live" className="w-full h-full object-cover opacity-90" />
                <div className="absolute top-2 left-2 bg-red-600/90 backdrop-blur-sm text-white text-[9px] font-bold px-2 py-1 rounded flex items-center gap-1.5 uppercase tracking-wider">
                  <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></div>
