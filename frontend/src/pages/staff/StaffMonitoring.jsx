...
export const StaffMonitoring = () => {
  ...
  const alerts = (securityAlerts || []).map(a => {
    let severity = 'Thấp';
    if (a.type === 'BIỂN SỐ ĐEN' || a.type?.includes('TRỘM')) {
      severity = 'Cao';
    } else if (a.type === 'TỪ CHỐI QUA CỔNG' || a.type === 'warning') {
      severity = 'Trung bình';
    }
    ...
    return {
      ...
      time: a.time,
      title: a.type,
      desc: `Biển số: ${a.plate || 'Không rõ'} • ${a.reason || a.description || ''}`,
      actionable: true
    };
  });
